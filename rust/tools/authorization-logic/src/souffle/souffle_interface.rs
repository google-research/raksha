...
/// Given a filename containing policy code this function parses it,
/// translates it to datalogIR, and emits souffle code to a new file.
pub fn input_to_souffle_file(input_file_path: &str, output_file_path: &str) {
    let source = fs::read_to_string(input_file_path)
        .unwrap_or_else(|err|panic!(
                "failed to read input in input_to_souffle_file: {}\n{}",
                                    input_file_path, err));
    let prog = astconstructionvisitor::parse_program(&source[..]);
    let universe_handled_prog = UniverseHandlingPass::handle_universes(&prog);
    let dlir_prog = LoweringToDatalogPass::lower(&universe_handled_prog);
    let souffle_code = souffle_emitter::emit_program(&dlir_prog, &None);
    fs::write(output_file_path, souffle_code)
...
/// main code.
pub fn ast_to_souffle_file(prog: &AstProgram, output_file_path: &str,
