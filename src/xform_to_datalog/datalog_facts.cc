  ...
  for (const arcs::RecipeProto &recipe_proto : manifest_proto.recipes()) {
    ...
    for (const arcs::ParticleProto &particle_proto : recipe_proto.particles()) {
      ...
      for (const arcs::HandleConnectionProto &connection_proto :
        ...
        // particle's handle connection. In the case that the handle
        // connection is input only, the edge out will be unreachable from
        // the rest of the graph. In the case that the connection is output
        // only, the edge in will be unreachable from the rest of the graph.
        // In the case that the connection is bidirectional, both edges will
        // be useful. Note that while the additional edges are, in some
        // sense, "wasted", the worst case is that we draw twice as many
        // edges as we need to, which doesn't affect asymptotic complexity.
        // In particular, the dataflow graph remains correct, if not as
