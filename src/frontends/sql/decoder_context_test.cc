...
TEST_P(DecodeMergeOpTest, DecodeMergeOpTest) {
  ...
  uint64_t next_id = 1;
  auto regester_value_and_get_id = [&decoder_context,
  ...
  auto direct_input_ids =
      MapIter<uint64_t>(direct_inputs, regester_value_and_get_id);
  auto control_input_ids =
      MapIter<uint64_t>(control_inputs, regester_value_and_get_id);
