...
class ManifestDatalogFactsToDatalogTest
    : public testing::TestWithParam<
          std::tuple<ManifestDatalogFacts, std::string>> {};
...
static const arcs::AccessPath kHandleH1AccessPath(
    arcs::AccessPathRoot(arcs::HandleAccessPathRoot("recipe", "h1")),
    ir::AccessPathSelectors());

static const arcs::AccessPath kHandleH2AccessPath(
    arcs::AccessPathRoot(arcs::HandleAccessPathRoot("recipe", "h2")),
    ir::AccessPathSelectors());

static const arcs::AccessPath kHandleConnectionInAccessPath(
    arcs::AccessPathRoot(
        arcs::HandleConnectionAccessPathRoot("recipe", "particle", "in")),
    ir::AccessPathSelectors());

static const arcs::AccessPath kHandleConnectionOutAccessPath(
    arcs::AccessPathRoot(
        arcs::HandleConnectionAccessPathRoot("recipe", "particle", "out")),
    ir::AccessPathSelectors());

std::vector<arcs::HandleConnectionSpec> GetHandleConnectionSpecs() {
  std::vector<arcs::HandleConnectionSpec> result;
  result.push_back(
      arcs::HandleConnectionSpec("in", /*reads=*/true, /*writes=*/false,
                                 /*type=*/type_factory.MakePrimitiveType()));
  result.push_back(
      arcs::HandleConnectionSpec("out", /*reads=*/false, /*writes=*/true,
                                 /*type=*/type_factory.MakePrimitiveType()));
  return result;
}

std::vector<arcs::TagCheck> GetTagChecks() {
  std::vector<arcs::TagCheck> result;
  result.push_back(arcs::TagCheck(kHandleConnectionInAccessPath,
                                  std::make_unique<arcs::TagPresence>("tag2")));
  return result;
}
...
static std::unique_ptr<arcs::ParticleSpec> particle_spec(
    arcs::ParticleSpec::Create(
        ...
        /*checks=*/GetTagChecks(),
        /*tag_claims=*/
        {arcs::TagClaim("particle", kHandleConnectionOutAccessPath, true,
                        "tag")},
        /*derives_from_claims=*/{},
...
// Edges:
        ...
        {ManifestDatalogFacts({ManifestDatalogFacts::Particle(
             ...
             /*instantiation_map*/
             {
                 {arcs::AccessPathRoot(arcs::HandleConnectionSpecAccessPathRoot(
                      "particle", "in")),
             ...
             /*edges*/
             {arcs::Edge(kHandleH1AccessPath, kHandleConnectionInAccessPath),
              arcs::Edge(kHandleConnectionOutAccessPath,
                         kHandleH2AccessPath)})}),
         R"(// Claims:
...
INSTANTIATE_TEST_SUITE_P(ManifestDatalogFactsToDatalogTest,
...
class ParseBigManifestTest : public testing::Test {
 ...
 protected:
  std::unique_ptr<arcs::SystemSpec> system_spec_;
  arcs::DatalogPrintContext ctxt_;
