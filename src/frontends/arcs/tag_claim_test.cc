...
class TagClaimToDatalogWithRootTest
    : public testing::TestWithParam<
          std::tuple<std::string,
                     std::tuple<std::string,
                                std::vector<absl::ParsedFormat<'s', 's', 's'>>>,
                     AccessPathRoot>> {};
...
TEST_P(TagClaimToDatalogWithRootTest, TagClaimToDatalogWithRootTest) {
  ...
  for (auto &format_str : expected_todatalog_format_string_vec) {
    expected_todatalog_vec.push_back(absl::StrFormat(
        format_str, particle_spec_name, root.ToString(), root.ToString()));
  }
  ::arcs::ClaimProto_Assume assume_proto;
  ASSERT_TRUE(google::protobuf::TextFormat::ParseFromString(assume_textproto,
                                                            &assume_proto));
  ...
  for (uint64_t i = 0; i < unrooted_tag_claim_vec.size(); ++i) {
    EXPECT_EQ(unrooted_tag_claim_vec.at(i).ToDatalog(ctxt),
  ...
  EXPECT_DEATH(unrooted_tag_claim_vec.at(0).ToDatalog(ctxt),
...
static std::string particle_spec_names[] = {"ParticleSpec1", "ParticleSpec2",
                                            "ps"};
...
static std::tuple<std::string, std::vector<absl::ParsedFormat<'s', 's', 's'>>> textproto_to_expected_format_string[] = {
    {R"(
access_path: {
...
predicate: { label: { semantic_tag: "tag2"} })",
     {absl::ParsedFormat<'s', 's', 's'>(
         R"(says_hasTag("%s", "%s", owner, "tag2") :- ownsAccessPath(owner, "%s").)")}},
    {R"(
access_path: {
  handle: { particle_spec: "ps", handle_connection: "hc" },
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Combine(testing::ValuesIn(particle_spec_names),
                     testing::ValuesIn(textproto_to_expected_format_string),
                     testing::ValuesIn(instantiated_roots)));

class TestTagClaimEquals
    : public testing::TestWithParam<
          std::tuple<std::tuple<std::string, AccessPath, bool, std::string>,
...
static AccessPath sample_access_paths[] = {
    ...
    AccessPath(
        ...
        ir::AccessPathSelectors(ir::Selector(ir::FieldSelector("field2"))))};

INSTANTIATE_TEST_SUITE_P(
    TestTagClaimEquals, TestTagClaimEquals,
    testing::Combine(testing::Combine(testing::ValuesIn(particle_spec_names),
