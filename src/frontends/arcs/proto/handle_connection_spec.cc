...
HandleConnectionSpec Decode(ir::types::TypeFactory &type_factory,
  ...
  switch (proto.direction()) {
    case ::arcs::HandleConnectionSpecProto_Direction_UNSPECIFIED:
      LOG(FATAL) << "Connection spec " << name << " has unspecified direction";
    ...
    case ::arcs::HandleConnectionSpecProto_Direction_READS_WRITES:
      ...
      break;
    default:
      LOG(FATAL) << "Unimplemented direction in connection spec " << name
                 << ".";
  }
  CHECK(proto.has_type()) << "Found connection spec " << name
                          << " without required type.";
  ir::types::Type type = ir::types::proto::Decode(type_factory, proto.type());
...
::arcs::HandleConnectionSpecProto Encode(const HandleConnectionSpec &hcs) {
  ...
  *result.mutable_type() = ir::types::proto::Encode(hcs.type());
  return result;
}
