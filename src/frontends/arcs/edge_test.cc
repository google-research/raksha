...
static const ir::AccessPathSelectors x_y_access_path_selectors =
...
static const std::tuple<Edge, std::string> edge_todatalog_pairs[] = {
    {Edge(AccessPath(AccessPathRoot(HandleConnectionAccessPathRoot(
                         "recipe", "particle", "handle")),
                     ir::AccessPathSelectors(
                         ir::Selector(ir::FieldSelector("field1")))),
          AccessPath(AccessPathRoot(HandleConnectionAccessPathRoot(
                         "recipe2", "particle2", "handle2")),
                     ir::AccessPathSelectors(
                         ir::Selector(ir::FieldSelector("field2"))))),
     R"(edge("recipe.particle.handle.field1", "recipe2.particle2.handle2.field2").)"},
    {Edge(AccessPath(
              AccessPathRoot(HandleConnectionAccessPathRoot("r", "p", "h")),
              x_y_access_path_selectors),
          AccessPath(
              AccessPathRoot(HandleConnectionAccessPathRoot("r", "p", "h2")),
              x_y_access_path_selectors)),
     R"(edge("r.p.h.x.y", "r.p.h2.x.y").)"},
    {Edge(AccessPath(
              AccessPathRoot(HandleConnectionAccessPathRoot("pre", "fix", "1")),
              ir::AccessPathSelectors()),
          AccessPath(
              AccessPathRoot(HandleConnectionAccessPathRoot("pre", "fix", "2")),
              ir::AccessPathSelectors())),
...
class EdgeEqTest : public testing::TestWithParam<
                       std::tuple<std::tuple<AccessPath, AccessPath>,
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Combine(testing::Combine(testing::ValuesIn(sample_access_paths),
