...
static const ProtoStringAndExpectations string_and_expections_array[] = {
    {.proto_str = R"(name: "foo" direction: READS type: { primitive: TEXT })",
     .expected_name = "foo",
     .expected_reads = true,
     .expected_writes = false},
    {.proto_str = R"(name: "bar" direction: WRITES type: { primitive: TEXT })",
     .expected_name = "bar",
     .expected_reads = false,
...
INSTANTIATE_TEST_SUITE_P(RoundTripHandleConnectionSpecProtoTest,
                         RoundTripHandleConnectionSpecProtoTest,
...
TEST_P(GetAccessPathTest, GetAccessPathTest) {
  ...
  for (const AccessPath &access_path : access_paths) {
    AccessPathRoot root = access_path.root();
    EXPECT_EQ(root, AccessPathRoot(HandleConnectionSpecAccessPathRoot(
...
type { entity: { schema: { fields: [
  ...
  { key: "hello", value: { primitive: TEXT } } ] } } })",
     ...
     .access_path_selectors_tostrings = {".field1", ".x.sub_field1",
                                         ".x.sub_field2", ".hello"}}};
