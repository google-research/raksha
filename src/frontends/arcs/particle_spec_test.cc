...
#include "src/frontends/arcs/proto/particle_spec.h"
...
class ParticleSpecFromProtoTest
 ...
 protected:
  ...
  static std::unique_ptr<ParticleSpec> CreateParticleSpec(
    ...
    CHECK(google::protobuf::TextFormat::ParseFromString(textproto,
                                                        &particle_spec_proto))
...
static ir::AccessPathSelectors MakeSingleFieldSelectors(
  ...
  return ir::AccessPathSelectors(
      ir::Selector(ir::FieldSelector(std::move(field_name))));
}
...
INSTANTIATE_TEST_SUITE_P(ParticleSpecFromProtoTest, ParticleSpecFromProtoTest,
