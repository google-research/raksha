...
static std::tuple<std::string, absl::ParsedFormat<'s'>>
    textproto_to_expected_format_string[] = {
      ...
      predicate: { label: { semantic_tag: "tag"} })",
         absl::ParsedFormat<'s'>(
             R"(checkP("check_num_0", $CP_TagPresenceDynamicOwner("%s.field1", "tag")).)")},
        {R"(access_path: {
      handle: { particle_spec: "ps", handle_connection: "hc" } },
      predicate: { label: { semantic_tag: "tag2"} })",
         absl::ParsedFormat<'s'>(
             R"(checkP("check_num_0", $CP_TagPresenceDynamicOwner("%s", "tag2")).)")},
        {R"(access_path: {
      handle: { particle_spec: "ps", handle_connection: "hc" },
      selectors: [{ field: "x" }, { field: "y" }] },
      predicate: { label: { semantic_tag: "user_selection"} })",
         absl::ParsedFormat<'s'>(
             R"(checkP("check_num_0", $CP_TagPresenceDynamicOwner("%s.x.y", "user_selection")).)")}};
