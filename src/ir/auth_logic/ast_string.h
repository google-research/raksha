  ...
  return std::visit(
      raksha::utils::overloaded{
          ...
          [](datalog::RelationDeclaration relationDeclaration) {
            ...
            for (const datalog::Argument& arg :
                 relationDeclaration.arguments()) {
          ...
          [](SaysAssertion saysAssertion) {
            ...
            return absl::StrCat(ToString(saysAssertion.principal()),
                                " says {\n",
                                absl::StrJoin(assertion_strings, "\n"), "}");
          },
          [](Query query) {
            return absl::StrCat("Query(", query.name(), " = ",
                                ToString(query.principal()), " says ",
                                ToString(query.fact()), "?)");
          },
          [](Program program) {
            std::vector<std::string> relation_decl_strings;
            relation_decl_strings.reserve(
                program.relation_declarations().size());
