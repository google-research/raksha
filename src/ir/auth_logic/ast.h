...
class VerbPhrase {
  public:
    static std::unique_ptr<VerbPhrase> 
    CreateVerbPhrase(Predicate pred) {
      return std::unique_ptr<VerbPhrase>(new VerbPhrase(std::move(pred)));
    }
    static std::unique_ptr<VerbPhrase>
    CreateVerbPhrase(Principal right_prin) {
      return std::unique_ptr<VerbPhrase>(new VerbPhrase(
            std::move(right_prin)));
    }
  private:
...
class FlatFact {
  public:
    static std::unique_ptr<FlatFact>
    CreateFlatFact(Principal prin, VerbPhrase verb_phrase) {
      return std::unique_ptr<FlatFact>(new FlatFact(
            VerbPhraseFlatFact(prin, verb_phrase)));
    }
    static std::unique_ptr<FlatFact>
    CreateFlatFact(Predicate prin) {
...
class Fact {
  public:
    static std::unique_ptr<Fact>
    CreateFact(FlatFact flat_fact) {
      return std::unique_ptr<Fact>(new Fact(std::move(flat_fact)));
    }
    static std::unique_ptr<Fact>
    CreateFact(Principal prin, Fact* fact) {
      return std::unique_ptr<Fact>(new Fact(CanSayFact(std::move(prin),
            std::move(fact))));
    }
  private:
    struct CanSayFact {
...
class Assertion {
  public:
    static std::unique_ptr<Assertion>
    CreateAssertion(Fact fact) {
      return std::unique_ptr<Assertion>(new Assertion(std::move(fact)));
    }
    static std::unique_ptr<Assertion>
    CreateAssertion(Fact lhs,
        std::vector<FlatFact> rhs) {
      return std::unique_ptr<Assertion>(new Assertion(
