...
INSTANTIATE_TEST_SUITE_P(SelectorTestSuite, SelectorTest,
...
TEST(SelectorHashTest, SelectorHashTest) {
  ...
  ASSERT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly(selectors_to_check));
}

class SelectorPairTest
...
TEST_P(SelectorPairTest, SelectorEqTest) {
  ...
  bool const expect_equal = (info1.selector_kind == info2.selector_kind) &&
...
INSTANTIATE_TEST_SUITE_P(
    SelectorPariTestSuite, SelectorPairTest,
    testing::Combine(testing::ValuesIn(example_selectors),
                     testing::ValuesIn(example_selectors)));
