...
class IRPrinter : public IRTraversingVisitor<IRPrinter> {
 public:
  ...
  Unit PreVisit(const Operation& operation) override {
    ...
    for (uint64_t i = 0; i < op.number_of_return_values(); ++i) {
      op_return_values.push_back(
          ssa_names_.GetOrCreateID(ir::Value::MakeOperationResultValue(
              operation, absl::StrCat(".", i))));
    }
