...
void IrProgramParser::ConstructOperation(
  ...
  value_map_.insert(
      {absl::StrCat(operation_context.VALUE_ID()->getText(), ".out"), v});
  ssa_names_.GetOrCreateID(&v, operation_context.VALUE_ID()->getText());
  ssa_names_.GetOrCreateID(&op, operation_context.VALUE_ID()->getText());
  std::cout << "Harsha" << ssa_names_.GetOrCreateID(&v) << std::endl;
  //  ssa_names_.UpdateID(Value(value::OperationResult(
  //            block_builder.AddOperation(
  //                *CHECK_NOTNULL(
  //                    context_.GetOperator(operation_context.ID()->getText())),
  //                std::move(attributes), std::move(inputs), nullptr),
  //            "out")), operation_context.VALUE_ID()->getText());
}
...
void IrProgramParser::ConstructBlock(IrParser::BlockContext& block_context) {
  ...
  ssa_names_.GetOrCreateID(std::addressof(b), block_context.ID()->getText());
}
...
void IrProgramParser::ConstructModule(IrParser::ModuleContext& module_context) {
  ...
  ssa_names_.GetOrCreateID(std::addressof(module_),
                           module_context.ID()->getText());
}
